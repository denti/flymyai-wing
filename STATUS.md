...
| Protection comes back | A wake re-arms, so "install it and it works" stays true past the first eight-hour lease. A wake is the only boundary that cannot defeat that lease, because reaching it requires the machine to have actually slept |
